...
class BedSVcall:
    ...
    def __init__(self, chrm, start, end, svtype, chrname2tid = None):
        self.chrm = chrm
        self.start = int(start)
        self.end = int(end) 
        self.svtype = svtype
...
def read_bedsvcall_file(in_bedsvcall_file, chrname2tid = None):

    in_bedsvcall_fp = open(in_bedsvcall_file, 'r')
    lines = list(in_bedsvcall_fp)
    in_bedsvcall_fp.close()

    bed_list = list()
    for line in lines:
        line = line.strip().split(tab)
        bed = BedSVcall(line[0], line[1], line[2], line[3], chrname2tid)
        bed_list.append(bed)
