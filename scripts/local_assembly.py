...
def process1region(samtools, bedtools, fermikit_dir, ref_fasta_file, input_bam_file, out_dir, itv, region_id, window_size, max_depth, n_threads, out_combined_vcf_file): 
    ...
    cmd = extract_bam_region(samtools, input_bam_file, interval, out_bam_file, n_threads)
    print(cmd)
    os.system(cmd)
    ...
    fermikit_variant_calling(fermikit_dir, samtools, n_threads, region_fasta_file, window_size, out_all_fastq_file, curr_out_dir, out_prefix)
...
def fermikit_variant_calling(fermikit_dir, samtools, n_threads, region_fasta_file, window_size, input_fastq_file, curr_out_dir, out_prefix):
...
def extract_bam_region(samtools, input_bam, interval, output_bam, n_threads):

    cmd = 'time %s view -1 -hb -@ %d %s %s > %s' % (samtools, n_threads, input_bam, interval, output_bam)
    return cmd
