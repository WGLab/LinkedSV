...
static BC_INFO_SET * read_bc_info(char * bcd_file, CHR_INFO * chr_info)
{
    ...
    i = 0;
    while (fgets(line, LINE_MAX, fp)){
        sscanf(line, "%d\t%d\t%d\t%d\t%s\t%d\t%d\n", &tid, &pos, &endpos, &mapq, bcd_seq, &hap_type, &flag);
        ...
        bc_info_set->bc_info[i].flag = flag;
...
static int output_files(BC_INFO_SET * bc_info_set, char * bcd21_file, CHR_INFO * chr_info)
{
    ...
    for (i = 0; i < bc_info_set->n_item; i++){
        ...
        fprintf (fp, "%d\t%d\t%d\t%d\t%s\t%d\t%d\t%d:%d\t%d:%d\t%d\t%d\n", bc_info[i].tid, bc_info[i].pos, endpos, mapq, bcd_seq, hap_type, bc_info[i].flag, bc_info[i].tid_prev, bc_info[i].pos_prev, bc_info[i].tid_next, bc_info[i].pos_next, dist_prev, dist_next);
    }
