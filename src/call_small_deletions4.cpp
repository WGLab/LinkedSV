...
{
public:
    double  score[4] = {0}; // variant score of the CNV call. 0, 1, and 2 for hap_type 0, 1 and 2. 3 for all hap_type. 
    uint32_t flag = 0;
    int cnv_hap_type; 
...
int output_cnv_candidate_regions(std::vector <CNVCall> & wg_candidate_cnv_region_vector, std::string out_file, int bin_size, CHR_INFO * chr_info)
{
    FILE * out_fp;
    char * chrname;
...
int calculate_depth_for_each_interval(std::string hap_type_read_depth_file, std::vector <std::vector <CNVInterval> > & wg_2d_interval_vector, CHR_INFO * chr_info, int bin_size)
{
    ...
    while (gzgets(hap_type_read_depth_fp, line, LOCAL_LINE_MAX))
    {
        ...
        ptr_itv->depth[3] += all_hap_total_read_depth * (double)l / (double)bin_size;
        ...
        ptr_itv->high_qual_depth[3] += all_hap_high_qual_read_depth  * (double)l / (double)bin_size;
...
int call_small_deletions(const std::string hap_type_read_depth_file, const std::string weird_reads_file, const std::string bcd22_file, const std::string faidx_file, const std::string blacklist_bed_file, const std::string out_file)
{
    ...
    /*
    output_all_regions(wg_2d_interval_vector, "all_regions.bed", bin_size, chr_info, wg_mean_base_generation_rate);

    output_cnv_candidate_regions(wg_candidate_cnv_region_vector, "cnv_candidate_regions.bed", bin_size, chr_info, wg_mean_base_generation_rate);

    output_cnv_candidate_regions(wg_r1_merged_cnv_region_vector, "round1_merged_regions.bed", bin_size, chr_info, wg_mean_base_generation_rate);
