...
def extract_weird_reads(args, dbo_args, endpoint_args): 
    ...
    cmd = "%s view %s | cut -f 1-6 > %s " % (args.samtools, args.sortn_bam, args.sortn_bam_core_file)
    ...
    args.weired_reads_file
    if args.run_from_begining == False and check_file_exists(args.weired_reads_file):
        myprint('File: %s existed, skipped %s' % (args.weired_reads_file, task))
    else:
        myprint(task)
        ...
        out_fp = open(args.weired_reads_file, 'w')
        ...
        while 1:
            ...
            if sam_core.read_id == current_read_id or current_read_id == '': 
                ...
                weired_read_info = find_weired_reads(sam_core_list, max_distance)

                if weired_read_info != '': out_fp.write(weired_read_info)
...
def find_weired_reads(sam_core_list, max_distance):
